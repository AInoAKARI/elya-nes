...
# THE MERGE: do the two wins compose?

Two branches were developed in parallel from the same base and both rewrote
`rom/nn.s`.

* **A**, the softmax branch: the quantised softmax's power-of-two normaliser
  hands out a realised sum anywhere in `(4, 8]` and landed on 4 in 25.4% of
  evaluations, so a quarter of the time the model ran at half its nominal
  probability resolution. Replacing it with `p = min(e*8/S, 7)` and
  re-laddering `AV_SHIFT` from 2 to 3.
* **B**, the mixture branch: the feed-forward block of every layer gets N
  copies on the cartridge, one of which streams per token, chosen by a 64-byte
  table on the current token id.

They are plausibly independent - A is a change to how attention weights are
spent, B is a change to how much feed-forward capacity exists - so the
plausible prediction is that the two loss improvements add. On this project a
prediction of exactly that shape has been refuted three times running (the
`AV_SHIFT` range argument, the widened probability nibble, the longer
context), so it was measured.

**They add.** Everything below is measured on the merged tree.

## 1. Resolving `rom/nn.s`: where each table has to live

The two branches' diffs are almost disjoint in `nn.s` - A is in `softmax`, the
AV chain and the table segment; B is in the bank map, the header walk and the
router - and `git merge` produced exactly one conflict hunk. That hunk is the
whole of the interesting part, because it is where the two mechanisms' TABLES
meet, and the resolution is not textual.

A mixture build cannot show the `$C000` window and an expert's header bank at
the same time, so every expert's header bank carries **its own copy** of the
lookup tables, `.assert`-ed to land at identical addresses. A adds three
tables. Which copy does each one belong in?

| table | size | read by | placement | why |
| --- | ---: | --- | --- | --- |
| `tbl_e8lo`, `tbl_e8hi` | `EXP_N` each | `softmax` | **per expert** | softmax runs with `$C000` showing the ROUTED bank |
| `tbl_p4` | `SM_TARGET` | `softmax` | **per expert** | same |
| `tbl_pv` | 256 | the AV chain | **once, fixed bank** | `PVTABLE` loads into `$E000`, which is mapped whichever expert won |

Getting either of those backwards produces a ROM that assembles. Put the exact
normaliser's tables in the fixed bank and a routed token reads its exp table
out of an unmapped window; duplicate `tbl_pv` per expert and 256 bytes a bank
are wasted for nothing. Both placements are generated by `host/ref.py` - the
per-expert copies into `moebanks.inc`, the `PVTABLE` segment into the
generated `nnmoe.cfg` - so there is one computation, not four hand-maintained
copies of a bank map.

The `$C000` table bank therefore holds **2,101** bytes of lookup tables under
the exact normaliser where the mixture branch measured 2,063, and a header
bank at N = 8 is 5,684 + 2,101 = 7,785 of 8,192. `TABLES_BYTES` in
`host/ref.py` is now computed from the normaliser rather than pinned at 2,063.

### The mistake this section nearly was

The first resolution of `rom/nn.s` was a `git checkout --ours` loop over the
conflicted paths, written to take `main`'s side of the BINARY artifacts before
regenerating them. `rom/nn.s` was in that list. The hand-resolved file was
silently replaced by `main`'s, the tree assembled cleanly, `build.sh` linked
every ROM, and the dense cartridge came out **1/19 tokens matching** - because
the packer was emitting the mixture branch's absolute-bank sentinel into a
header table that `main`'s `read_header` still interpreted as an increment.

It was caught in one step, by running the ROM. It would not have been caught
by reading the diff, by the assembler, or by the linker. The control that
found it was building `main` and `moe` alone in scratch worktrees and
confirming that both give 19/19 on this box before believing anything about
the merge.

## 2. Regression: both branches' shipping cartridges, rebuilt here

| arm | published | rebuilt on the merged tree |
| --- | --- | --- |
| A: dense, exact, `AV_SHIFT` 3 | 1,129,291 cycles, 19/19 | **1,129,375**, 19/19 EXACT |
| B: 8 experts, pow2, `AV_SHIFT` 2 | 1,123,138 cycles, 19/19 | **1,123,138**, 19/19 EXACT |

B is bit-identical. A costs **+84 cycles**, and it is accounted for rather
than waved at: the mixture branch's sentinel carries an absolute bank number
instead of meaning "increment", which a dense walk pays for twice - one extra
opening bank-select row at 71 cycles (the measured in-situ switch body) plus 2
cycles on each of the six switches a dense token already made. 83 predicted,
84 measured. The mixture branch had already measured the same +84 on its own
power-of-two dense arm (1,116,979 -> 1,117,063); it reproduces here on the
exact-normalisation arm, which is what it should do, because the sentinel is
in the header walk and the normaliser is in softmax.

## 3. The composition control that needs no retraining

`train/replicate_experts.py` turns the softmax branch's own shipping npz into
a 16-expert npz whose experts hold **identical** weights, routed `tok % 16` so
the routed bank changes as often as it can. Every mixture mechanism runs - the
router, 16 header tables, 16 copies of the lookup tables including the exact
normaliser's three, 87 stream banks, the absolute sentinel leaving the shared
region and coming back three times a token - while the arithmetic stays the
dense model's. So the output must be the dense output, exactly.

```
dense  cartridge tokens : [1, 4, 2, 0, 20, 18, 33, 0, 44, 18, 0, 8, 3, 58, 29, 24, 49, 26, 2, 0]
16-expert same weights  : [1, 4, 2, 0, 20, 18, 33, 0, 44, 18, 0, 8, 3, 58, 29, 24, 49, 26, 2, 0]
ROM vs host             : 19/19 EXACT
cycles/token            : 1,128,243 against the dense 1,129,375
```

The 1,132-cycle difference is bank-boundary layout, not arithmetic - an expert
region's chunks are bank-aligned, so the walk crosses a different number of
boundaries. The mixture branch's own 4-identical-expert control moved the same
way and the same sign.

This says the two mechanisms compose **in the ROM**. It says nothing about
whether they compose in the LOSS, which is a different question with a
different answer available only by training.

## 4. The 2 x 2

Four cells, 60,000 steps, two seeds each, bpe64 / tau 0.75 / batch 192 /
lr 3e-3 / balanced routing throughout, everything else identical, all trained
on this tree (`train/factorial.sh`).

All four cells were re-run here rather than compared against the two branches'
published numbers, because the mixture branch had already recorded that its
own dense control (1.4020 / 1.4096) sits 0.008 nats/char from the number the
context branch published for the same recipe (1.4133 / 1.4149) - and 0.008 is
the same size as the interaction term the question is about.

Scored by the trainer's own final 60-batch evaluation, which is the estimator
both branches published with:

| val nats/char | pow2 + `AV_SHIFT` 2 | exact + `AV_SHIFT` 3 |
| --- | ---: | ---: |
| dense | 1.4020 / 1.4096 = **1.4058** | 1.3754 / 1.3779 = **1.3766** |
| 8 experts | 1.2202 / 1.2221 = **1.2211** | 1.1932 / 1.1898 = **1.1915** |

```
main effect A, exact normalisation + AV_SHIFT 3
    at dense capacity     -0.0292   (1.4058 -> 1.3766)
    at 8 experts          -0.0296   (1.2211 -> 1.1915)
main effect B, 8 experts
    under pow2            -0.1847   (1.4058 -> 1.2211)
    under exact           -0.1851   (1.3766 -> 1.1915)

additive prediction   1.4058 -0.0292 -0.1847 = 1.1920
measured merged cell                           1.1915
INTERACTION                                   -0.0005
seed spread within a cell                      0.0076
```

Re-scored by `train/eval_npz.py` (60 batches, evaluation generator reset
rather than carried through training - a different unbiased draw from the same
held-out split, which shifts every LEVEL by about 0.008 nats/char), the same
arithmetic gives an interaction of **-0.0039** against a seed spread of
0.0084.

**Both interactions are inside the seed spread, and both have the sign of very
slightly MORE than additive.** There is no detectable interference. A softmax
that stops wasting a quarter of its budget is worth the same 0.029 nats/char
whether the feed-forward has one copy or eight; eight copies are worth the
same 0.185 whichever normaliser is spending the budget.

Each arm was evaluated in a subprocess carrying the environment it was trained
under, because `train/model_nes.py` builds its forward pass from
`NES_SM_NORM` / `NES_AV_SHIFT` at import: scoring a power-of-two model through
the exact-normalisation forward pass gives a plausible number and that number
is what the interaction is made of. The trainer now stamps `_smnorm` into the
npz and `host/ref.py` and `train/eval_npz.py` both refuse a mismatch, so the
table can fail but cannot quietly be wrong.

### Two exact reproductions, and one number that does not reproduce

`n8_pow2` reproduces the mixture branch's 1.2202 / 1.2221 to every digit its
json holds - fit, val, nonzero count and the learned logit scale all identical
- and `dense_pow2` reproduces its dense control 1.4020 / 1.4096 the same way.
`dense_exact` reproduces the softmax branch's 1.3754 / 1.3779 exactly.

Which makes the one number that does not reproduce worth stating plainly.
**The softmax branch's headline of -0.0375 nats/char is 0.008 too large.** It
is measured against a dense + pow2 baseline of 1.4141 that came from
`runs/final_av2_bpe64_tau0.75` and `runs/t20_final_s2`, two arms that predate
the `_ctx` stamp and were produced by an older implementation of the same
forward pass. Against a dense + pow2 control trained on this tree the same
effect is **-0.0292**, and it is -0.0296 at 8 experts. The effect is real,
non-overlapping, and reproduces at both capacities; it is smaller than
published, and the reason is a baseline, not the mechanism.

## 5. The cycles add too

Each cell's own 60,000-step seed-1 cartridge, MAME 0.277, 19 tokens, mean:

| cycles/token | pow2 + `AV_SHIFT` 2 | exact + `AV_SHIFT` 3 | A costs |
| --- | ---: | ---: | ---: |
| dense | 1,117,248 | 1,129,375 | +12,127 (+1.085%) |
| 8 experts | 1,123,138 | **1,134,432** | +11,294 (+1.006%) |
| B costs | +5,890 (+0.527%) | +5,057 (+0.448%) | |

```
additive prediction   1,117,248 +12,127 +5,890 = 1,135,265
measured                                         1,134,432
INTERACTION                                         -833 cycles, -0.075%
```

All four cells are 19/19 EXACT. The merged cartridge costs **+1.538%** against
dense + power-of-two and buys **-0.214 nats/char**.

## 6. Every gate on the merged cartridge

`train/moe_gate.sh runs/fac/n8_exact_s1.npz MERGED`:

| gate | result |
| --- | --- |
| build variants link | **24 / 24**, 0 failures |
| `max\|dW\|` | **0** over 446,464 ternary weights, decoded back out of the stream |
| ROM vs host, seed token 1 | **19/19 EXACT** |
| 64-seed survey | **1,216 / 1,216 EXACT** |
| expert coverage | **every expert routed** - 224/170/158/185/122/140/115/166 of 1,280 token-positions; 6 to 8 distinct experts per seed, mean 7.44 |
| block 16 | 88,271 blocks, max \|v\| = **203**, **0** over 127 signed, **0** over 255 biased (provable bound 224) |
| block 32 | max 378, 43 signed and 6,306 biased overflows - still refuted, now on a mixture |
| cycles/token | 1,134,432 mean; profiled 1,134,652 (pos 0) to 1,198,607 (pos 18) |
...
then be untested. The `-DBANKPROF` build was re-run because 11 cycles was
measured under the power-of-two normaliser; it reproduces under the exact one,
which it should, since the router runs before the first layer and the
normaliser runs inside softmax.

The survey itself was hardened first: it links 64 times and piped every one of
them through `grep`, so a link failure would have left the previous
`out/sv.nes` on disk to be measured 64 times and reported as 1,216/1,216.
That is the same pipe `build.sh` had already had removed.

## 7. What the merge makes false in the two journals

* **"-0.0375 nats/char" (README, and the softmax journal's headline).** It is
  -0.0292 against a matched control on one tree. See section 4.
* **"the lookup tables are 2,063 bytes" / "8,143 of 8,192, 241 to spare"**
  (the mixture journal's bank-occupancy table, and DESIGN). Under the exact
  normaliser they are 2,101, and a header bank at N = 8 is 7,785 of 8,192.
  DESIGN is corrected in place; the journal entry is left standing with this
  note, because it was true when it was measured.
* **"1,116,979 cycles/token" as the dense baseline** (the mixture journal's
  README table). The dense baseline on a tree with the absolute sentinel is
  1,117,063 for that arm, and 1,117,248 for the dense + pow2 arm trained here.
  The mixture journal's own text already gives 1,117,063; only its summary
  table still says 1,116,979.
* **"the shipping normalisation"** in DESIGN's softmax section now describes a
  configuration that is also a mixture; the sentence is still true and is left
  alone.
* Nothing in the mixture journal's routing, bank-budget, or 71/73-cycle
  results is affected. Nothing in the softmax journal's screen (the widened
  nibble buying nothing) or its context re-test is affected.
...
* **The 2 x 2 is 2 seeds a cell, not more.** The interaction term is -0.0005
  ...
  -0.0005 from 0 would need several times the runs.
* **N = 16 was not retrained under the exact normaliser.** The 16-expert
  ceiling result stays a power-of-two, single-seed number. What was checked
  here is only that the geometry still works: 16 identical experts pack, link
  and run bit-exactly under the exact normaliser.
* **`T = 85` was not retrained either**, under either normaliser at
  `AV_SHIFT = 3`. It links and packs; no mixture has ever been trained at
  `T = 85`.
* **Nothing was done about the 0.008 nats/char between the old and current
  implementations of the dense + pow2 arm.** Both are on the tree, both are
  reproducible, and the difference is recorded rather than explained. The
  older arms predate the `_ctx` stamp, which places them before the context
  work, but which change moved the number was not chased.
* **The `.sav` result block was not re-verified** on the merged cartridge; the
  mixture branch verified it on its own.
