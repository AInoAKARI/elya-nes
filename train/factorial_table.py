...
def main():
    ...
    mean = {}
    for cap, norm in CELLS:
        v = [got[(cap, norm, s)][1] for s in SEEDS if (cap, norm, s) in got]
        if not v:
            continue
    ...
    need = [("dense", "pow2"), ("dense", "exact"), ("n8", "pow2"), ("n8", "exact")]
    if not all(k in mean for k in need):
        print("\n(incomplete: the interaction needs all four cells)")
        return
    dp, de, np_, ne = (mean[k] for k in need)

    # main effects, each measured at BOTH levels of the other factor
    a_dense = de - dp            # softmax effect with dense capacity
    a_moe = ne - np_             # softmax effect with 8 experts
    ...
    print("""
seed spread within a cell (max |seed1 - seed2|): %.4f
...
""" % (max(abs(got[(c, n, 1)][1] - got[(c, n, 2)][1])
           for c, n in CELLS if (c, n, 2) in got),
    ...
    print("\npublished elsewhere, for reference only (different trees):")
    print("  dense + pow2   1.4141    dense + exact  1.3766")
    print("  8 experts + pow2  1.2202 / 1.2221")
